...
import {
  activeTimePercent,
  activeTimePeriods,
  activeTimePeriodSubset,
  channelTimePeriods,
  gcdTimePeriods,
} from 'parser/shared/metrics/activeTime';

/** Testing info with fight from 0 to 1000, and filled in with dummy values */
function _info(): Info {
  return {
    playerId: 1,
    pets: [],
...
describe('activeTimePeriodSubset', () => {
  const info = _info();
  it('simple in subset', () => {
    const events: AnyEvent[] = [_endChannel(1, 300, 200)];
    expect(activeTimePeriodSubset(events, info, ctp(0, 500))).toEqual([ctp(200, 300)]);
  });
  it('many times', () => {
    const events: AnyEvent[] = [
      _endChannel(1, 300, 200),
